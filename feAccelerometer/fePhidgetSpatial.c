...
{
  int serialNo;
  Phidget_getDeviceSerialNumber((PhidgetHandle)ch, &serialNo);
...
{ 
    ...
    printf("Call onSpatial0_SpatialData\n");
    int count=50;
  int i;
  for(i = 0; i < count; i++) { // TL This loop doesn't do anything... just loops over same variables 50 times.
...
INT frontend_init()
{
  ...
  PhidgetAccelerometer_create(&accelerometer0);
  Phidget_setDeviceSerialNumber((PhidgetHandle)spatial0, serial_number);  // TL added this.
