...
#define STORE_DATA(TYPE) {
  ...
  /* Rika (31Mar2017): INSERTED NEW GLOBAL CALCULATOR OBJECTS */
  // Gantry dimensions
  double tiltMotorLength = 0.160;
    double gantryFrontHalfLength = 0.140; //Use 0.140m for safety
  ...
  double xtankCentre = 0.374; // Rika (7Apr2017): Updated to new centre of gantry system, for B-field studies.
  double ytankCentre = 0.348;
  ...
  double pmtXcentre0 = 0.374; // Rika (7Apr2017): Updated to new centre of gantry system.
  double pmtYcentre0 = 0.348;
  double pmtXcentre1 = 0.374;
  double pmtYcentre1 = 0.348;
  int numPolySides = 12;
  ...
  double pmtHeight = 0.590; //Rika (21Mar2017): actual PMT height in gantry coordinates;
                            // Gantry tilted -90 will hit PMT cover at z=0.475m.
...
void move_init(HNDLE hDB,HNDLE hKey,void *data){
  ...
  // FOR DEBUGGING
  //cm_msg(MINFO, "move_init", "Preparing call to generate_path: Initializing PMT position."); 

  // PMT position: to be used in generate_path for PMT collision avoidance.
  XYPolygon poly0, poly1;
  for(int polyNum = 1; polyNum <= 11; ++polyNum){
      poly0.clear();
      poly1.clear();
...
int generate_path(INFO *pInfo){
  ...
  double safeZheight = 0.460; // Rika (4Apr2017): z height at which any movement (rot & tilt) is PMT collision free.
