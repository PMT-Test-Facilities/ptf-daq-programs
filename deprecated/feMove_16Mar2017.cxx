/********************************************************************	\
  ...
  27-NOV-12    NSA   Implemented single arm simple motion
   6-FEB-12		 NSA	 Cleaned up a bit, generalized some functionality
										 for use with 2 arms
  14-Mar-17    Rika  Adding PMT polygon object for improved collision avoidance (not yet complete)
\********************************************************************/
#include <stdlib.h>
#include <stdio.h>
#include "midas.h"
...
#define STORE_DATA(TYPE) {
  ...
  void getGantryConfigurations(double gantryXDimensions[], double gantryYDimensions[], int whichGantry, double rot, double tilt);
    // void getPMTpolygon(std::vector< std::pair<double, double> > poly, double pmtRadius, int polyNum, double pmtXcentre, double pmtYcentre, int numPolySides, double polyHeight);
...
// 		-the eight possible permutations that system can undertake are: 1) gantry 1 move first, rotation first, tilt first; 
...
int generate_path(INFO *pInfo){
  ...
  //double pmtHeight = 0.9;  //use when PMT is not in place
  double pmtHeight = 0.46; //TODO: check this value for all tilt angles. Currently trying to be safe//0.482;//use when PMT in place

  double z_max_value = 0.57; // Rika: Gantry limit in z, taken from ScanSequence.cxx

  // Rika: PMT position for collision avoidance
  std::vector< std::pair<double, double> > pmtPoly0; // Model of PMT for collision avoidance
  double pmtRadius = 0.323;
  double pmtXcentre0 = 0.354;
  double pmtYcentre0 = 0.401;
  double pmtXcentre1 = 0.354;
  double pmtYcentre1 = 0.429;
  int numPolySides = 12;
  ...
  TPathCalculator pathCalc5a;
  TPathCalculator pathCalc5b;
  TPathCalculator pathCalc6a;
  TPathCalculator pathCalc6b;
  TPathCalculator pathCalc7a;
  TPathCalculator pathCalc7b;
  TPathCalculator pathCalc8a;
  TPathCalculator pathCalc8b;
  TRotationCalculator pathCalc_rotstart_tiltfirst;
  TRotationCalculator pathCalc_rotend_tiltfirst;
  TRotationCalculator pathCalc_rotend_check_tiltfirst;
  TRotationCalculator pathCalc_rotstart_tiltsecond;
  TRotationCalculator pathCalc_rotend_tiltsecond;
  TRotationCalculator pathCalc_rotend_check_tiltsecond;
  ...
  //       the four corners of the gantry area are defined in the array with the following indices:
  //         0 := back right point
  //         1 := front right point
  //         2 := front left point
  //         3 := back left point
  ...
  double gantry1XDimensions_rotfirst_tiltfirst[4];
  double gantry1YDimensions_rotfirst_tiltfirst[4];
  double opticalBox0XDimensions_rotfirst_tiltfirst[4];
  double opticalBox0YDimensions_rotfirst_tiltfirst[4];
  double gantry2XDimensions_rotfirst_tiltfirst[4];
  double gantry2YDimensions_rotfirst_tiltfirst[4];
  double opticalBox1XDimensions_rotfirst_tiltfirst[4];
  double opticalBox1YDimensions_rotfirst_tiltfirst[4];

  double z0_rotfirst_tiltfirst;
  double z1_rotfirst_tiltfirst;

  // Gantry XY corners in global coordinates, when at the gantries limit switches, with initial rotation conditions
  double gantry1XDimensions_rotsecond_tiltfirst[4];
  double gantry1YDimensions_rotsecond_tiltfirst[4];
  double opticalBox0XDimensions_rotsecond_tiltfirst[4];
  double opticalBox0YDimensions_rotsecond_tiltfirst[4];
  double gantry2XDimensions_rotsecond_tiltfirst[4];
  double gantry2YDimensions_rotsecond_tiltfirst[4];
  double opticalBox1XDimensions_rotsecond_tiltfirst[4];
  double opticalBox1YDimensions_rotsecond_tiltfirst[4];

  double z0_rotsecond_tiltfirst;
  double z1_rotsecond_tiltfirst;

  // Tilt second
  // Gantry XY corners in global coordinates, when at the gantries limit switches, with final rotation conditions
  double gantry1XDimensions_rotfirst_tiltsecond[4];
  double gantry1YDimensions_rotfirst_tiltsecond[4];
  double opticalBox0XDimensions_rotfirst_tiltsecond[4];
  double opticalBox0YDimensions_rotfirst_tiltsecond[4];
  double gantry2XDimensions_rotfirst_tiltsecond[4];
  double gantry2YDimensions_rotfirst_tiltsecond[4];
  double opticalBox1XDimensions_rotfirst_tiltsecond[4];
  double opticalBox1YDimensions_rotfirst_tiltsecond[4];

  double z0_rotfirst_tiltsecond;
  double z1_rotfirst_tiltsecond;

  // Gantry XY corners in global coordinates, when at the gantries limit switches, with initial rotation conditions
  double gantry1XDimensions_rotsecond_tiltsecond[4];
  double gantry1YDimensions_rotsecond_tiltsecond[4];
  double opticalBox0XDimensions_rotsecond_tiltsecond[4];
  double opticalBox0YDimensions_rotsecond_tiltsecond[4];
  double gantry2XDimensions_rotsecond_tiltsecond[4];
  double gantry2YDimensions_rotsecond_tiltsecond[4];
  double opticalBox1XDimensions_rotsecond_tiltsecond[4];
  double opticalBox1YDimensions_rotsecond_tiltsecond[4];

  double z0_rotsecond_tiltsecond;
  double z1_rotsecond_tiltsecond;
  ...
  // Possible gantry 1 & 2 configurations
  // Rika (March7,2017): Implemented getGantryConfigurations function instead of code commented out below.
  // Rika (March14, 2017): Added calculation of optical box configuration in getGantryConfigurations
  // Tilt first
  // Final state of gantry1 & gantry2, after rotation with XY limit corners with tilt dependence
  getGantryConfigurations(gantry1XDimensions_rotfirst_tiltfirst,
			  gantry1YDimensions_rotfirst_tiltfirst,
			  1, gant1_rot1, gant1_tilt_start);
  getGantryConfigurations(gantry2XDimensions_rotfirst_tiltfirst,
						  gantry2YDimensions_rotfirst_tiltfirst,
						  2, gant2_rot1, gant2_tilt_start);
  // Final state of gantry1 & gantry2, before rotation with XY limit corners with tilt dependence
  getGantryConfigurations(gantry1XDimensions_rotsecond_tiltfirst,
						  gantry1YDimensions_rotsecond_tiltfirst,
						  1, gant1_rot2, gant1_tilt_start);
  getGantryConfigurations(gantry2XDimensions_rotsecond_tiltfirst,
						   gantry2YDimensions_rotsecond_tiltfirst,
						   2, gant2_rot2, gant2_tilt_start);
  // Tilt second
  //Final state of gantry1 & gantry2, after rotation with XY limit corners with tilt dependence
  getGantryConfigurations(gantry1XDimensions_rotfirst_tiltsecond,
						   gantry1YDimensions_rotfirst_tiltsecond,
						   1, gant1_rot1, gant1_tilt_end);
  getGantryConfigurations(gantry2XDimensions_rotfirst_tiltsecond,
						   gantry2YDimensions_rotfirst_tiltsecond, 
						   2, gant2_rot1, gant1_tilt_end);
  // Final state of gantry1 & gantry2, before rotation with XY limit corners with tilt dependence
  getGantryConfigurations(gantry1XDimensions_rotsecond_tiltsecond,
						    gantry1YDimensions_rotsecond_tiltsecond,
						    1, gant1_rot2, gant1_tilt_end);
  getGantryConfigurations(gantry2XDimensions_rotsecond_tiltsecond,
						   gantry2YDimensions_rotsecond_tiltsecond,
						   2, gant2_rot2, gant1_tilt_end);

  /**Below is the original code for determining gantry 1 & 2 configurations, now implemented by the function getGantryConfigurations
  // Tilt first
  // Final state of gantry1 & gantry2, after rotation with XY limit corners with tilt dependence
  gantry1YDimensions_rotfirst_tiltfirst[0] = cos(gant1_tilt_start)*(-0.070*cos(gant1_rot1)-0.200*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(-0.070*cos(gant1_rot1)-0.110*sin(gant1_rot1));
  gantry1XDimensions_rotfirst_tiltfirst[0] = cos(gant1_tilt_start)*(-0.200*cos(gant1_rot1)+0.070*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(-0.110*cos(gant1_rot1)+0.070*sin(gant1_rot1));
  gantry1YDimensions_rotfirst_tiltfirst[1] = cos(gant1_tilt_start)*(-0.070*cos(gant1_rot1)+0.140*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(-0.070*cos(gant1_rot1)+0.110*sin(gant1_rot1));//Front of Gantry
  gantry1XDimensions_rotfirst_tiltfirst[1] = cos(gant1_tilt_start)*(0.140*cos(gant1_rot1)+0.070*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(0.110*cos(gant1_rot1)+0.070*sin(gant1_rot1));//Front of Gantry
  gantry1YDimensions_rotfirst_tiltfirst[2] = cos(gant1_tilt_start)*(0.160*cos(gant1_rot1)+0.140*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(0.160*cos(gant1_rot1)+0.110*sin(gant1_rot1));//Front of Gantry
  gantry1XDimensions_rotfirst_tiltfirst[2] = cos(gant1_tilt_start)*(0.140*cos(gant1_rot1)-0.160*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(0.110*cos(gant1_rot1)-0.160*sin(gant1_rot1));//Front of Gantry
  gantry1YDimensions_rotfirst_tiltfirst[3] = cos(gant1_tilt_start)*(0.160*cos(gant1_rot1)-0.200*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(0.160*cos(gant1_rot1)-0.110*sin(gant1_rot1));
  gantry1XDimensions_rotfirst_tiltfirst[3] = cos(gant1_tilt_start)*(-0.200*cos(gant1_rot1)-0.160*sin(gant1_rot1)) + (1-cos(gant1_tilt_start))*(-0.110*cos(gant1_rot1)-0.160*sin(gant1_rot1));

  gantry2YDimensions_rotfirst_tiltfirst[0] = cos(gant2_tilt_start)*(0.070*cos(gant2_rot1)+0.200*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(0.070*cos(gant2_rot1)+0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltfirst[0] = cos(gant2_tilt_start)*(0.200*cos(gant2_rot1)-0.070*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(0.110*cos(gant2_rot1)-0.070*sin(gant2_rot1));
  gantry2YDimensions_rotfirst_tiltfirst[1] = cos(gant2_tilt_start)*(0.070*cos(gant2_rot1)-0.140*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(0.070*cos(gant2_rot1)-0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltfirst[1] = cos(gant2_tilt_start)*(-0.140*cos(gant2_rot1)-0.070*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(-0.110*cos(gant2_rot1)-0.070*sin(gant2_rot1));
  gantry2YDimensions_rotfirst_tiltfirst[2] = cos(gant2_tilt_start)*(-0.160*cos(gant2_rot1)-0.140*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(-0.160*cos(gant2_rot1)-0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltfirst[2] = cos(gant2_tilt_start)*(-0.140*cos(gant2_rot1)+0.160*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(-0.110*cos(gant2_rot1)+0.160*sin(gant2_rot1));
  gantry2YDimensions_rotfirst_tiltfirst[3] = cos(gant2_tilt_start)*(-0.160*cos(gant2_rot1)+0.200*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(-0.160*cos(gant2_rot1)+0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltfirst[3] = cos(gant2_tilt_start)*(0.200*cos(gant2_rot1)+0.160*sin(gant2_rot1))+(1-cos(gant2_tilt_start))*(0.110*cos(gant2_rot1)+0.160*sin(gant2_rot1));

  // Final state of gantry1 & gantry2, before rotation with XY limit corners w/ tilt dependence
  gantry1YDimensions_rotsecond_tiltfirst[0] = cos(gant1_tilt_start)*(-0.070*cos(gant1_rot2)-0.200*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(-0.070*cos(gant1_rot2)-0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltfirst[0] = cos(gant1_tilt_start)*(-0.200*cos(gant1_rot2)+0.070*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(-0.110*cos(gant1_rot2)+0.070*sin(gant1_rot2));
  gantry1YDimensions_rotsecond_tiltfirst[1] = cos(gant1_tilt_start)*(-0.070*cos(gant1_rot2)+0.140*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(-0.070*cos(gant1_rot2)+0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltfirst[1] = cos(gant1_tilt_start)*(0.140*cos(gant1_rot2)+0.070*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(0.110*cos(gant1_rot2)+0.070*sin(gant1_rot2));
  gantry1YDimensions_rotsecond_tiltfirst[2] = cos(gant1_tilt_start)*(0.160*cos(gant1_rot2)+0.140*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(0.160*cos(gant1_rot2)+0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltfirst[2] = cos(gant1_tilt_start)*(0.140*cos(gant1_rot2)-0.160*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(0.110*cos(gant1_rot2)-0.160*sin(gant1_rot2));
  gantry1YDimensions_rotsecond_tiltfirst[3] = cos(gant1_tilt_start)*(0.160*cos(gant1_rot2)-0.200*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(0.160*cos(gant1_rot2)-0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltfirst[3] = cos(gant1_tilt_start)*(-0.200*cos(gant1_rot2)-0.160*sin(gant1_rot2))+(1-cos(gant1_tilt_start))*(-0.110*cos(gant1_rot2)-0.160*sin(gant1_rot2));

  gantry2YDimensions_rotsecond_tiltfirst[0] = cos(gant2_tilt_start)*(0.070*cos(gant2_rot2)+0.200*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(0.070*cos(gant2_rot2)+0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltfirst[0] = cos(gant2_tilt_start)*(0.200*cos(gant2_rot2)-0.070*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(0.110*cos(gant2_rot2)-0.070*sin(gant2_rot2));
  gantry2YDimensions_rotsecond_tiltfirst[1] = cos(gant2_tilt_start)*(0.070*cos(gant2_rot2)-0.140*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(0.070*cos(gant2_rot2)-0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltfirst[1] = cos(gant2_tilt_start)*(-0.140*cos(gant2_rot2)-0.070*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(-0.110*cos(gant2_rot2)-0.070*sin(gant2_rot2));
  gantry2YDimensions_rotsecond_tiltfirst[2] = cos(gant2_tilt_start)*(-0.160*cos(gant2_rot2)-0.140*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(-0.160*cos(gant2_rot2)-0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltfirst[2] = cos(gant2_tilt_start)*(-0.140*cos(gant2_rot2)+0.160*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(-0.110*cos(gant2_rot2)+0.160*sin(gant2_rot2));
  gantry2YDimensions_rotsecond_tiltfirst[3] = cos(gant2_tilt_start)*(-0.160*cos(gant2_rot2)+0.200*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(-0.160*cos(gant2_rot2)+0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltfirst[3] = cos(gant2_tilt_start)*(0.200*cos(gant2_rot2)+0.160*sin(gant2_rot2))+(1-cos(gant2_tilt_start))*(0.110*cos(gant2_rot2)+0.160*sin(gant2_rot2));

  // Tilt second
  //Final state of gantry1 & gantry2, after rotation with XY limit corners with tilt dependence
  gantry1YDimensions_rotfirst_tiltsecond[0] = cos(gant1_tilt_end)*(-0.070*cos(gant1_rot1)-0.200*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(-0.070*cos(gant1_rot1)-0.110*sin(gant1_rot1));
  gantry1XDimensions_rotfirst_tiltsecond[0] = cos(gant1_tilt_end)*(-0.200*cos(gant1_rot1)+0.070*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(-0.110*cos(gant1_rot1)+0.070*sin(gant1_rot1));
  gantry1YDimensions_rotfirst_tiltsecond[1] = cos(gant1_tilt_end)*(-0.070*cos(gant1_rot1)+0.140*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(-0.070*cos(gant1_rot1)+0.110*sin(gant1_rot1));
  gantry1XDimensions_rotfirst_tiltsecond[1] = cos(gant1_tilt_end)*(0.140*cos(gant1_rot1)+0.070*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(0.110*cos(gant1_rot1)+0.070*sin(gant1_rot1));
  gantry1YDimensions_rotfirst_tiltsecond[2] = cos(gant1_tilt_end)*(0.160*cos(gant1_rot1)+0.140*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(0.160*cos(gant1_rot1)+0.110*sin(gant1_rot1));
  gantry1XDimensions_rotfirst_tiltsecond[2] = cos(gant1_tilt_end)*(0.140*cos(gant1_rot1)-0.160*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(0.110*cos(gant1_rot1)-0.160*sin(gant1_rot1));
  gantry1YDimensions_rotfirst_tiltsecond[3] = cos(gant1_tilt_end)*(0.160*cos(gant1_rot1)-0.200*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(0.160*cos(gant1_rot1)-0.110*sin(gant1_rot1));
  gantry1XDimensions_rotfirst_tiltsecond[3] = cos(gant1_tilt_end)*(-0.200*cos(gant1_rot1)-0.160*sin(gant1_rot1))+(1-cos(gant1_tilt_end))*(-0.110*cos(gant1_rot1)-0.160*sin(gant1_rot1));

  gantry2YDimensions_rotfirst_tiltsecond[0] = cos(gant2_tilt_end)*(0.070*cos(gant2_rot1)+0.200*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(0.070*cos(gant2_rot1)+0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltsecond[0] = cos(gant2_tilt_end)*(0.200*cos(gant2_rot1)-0.070*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(0.110*cos(gant2_rot1)-0.070*sin(gant2_rot1));
  gantry2YDimensions_rotfirst_tiltsecond[1] = cos(gant2_tilt_end)*(0.070*cos(gant2_rot1)-0.140*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(0.070*cos(gant2_rot1)-0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltsecond[1] = cos(gant2_tilt_end)*(-0.140*cos(gant2_rot1)-0.070*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(-0.110*cos(gant2_rot1)-0.070*sin(gant2_rot1));
  gantry2YDimensions_rotfirst_tiltsecond[2] = cos(gant2_tilt_end)*(-0.160*cos(gant2_rot1)-0.140*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(-0.160*cos(gant2_rot1)-0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltsecond[2] = cos(gant2_tilt_end)*(-0.140*cos(gant2_rot1)+0.160*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(-0.110*cos(gant2_rot1)+0.160*sin(gant2_rot1));
  gantry2YDimensions_rotfirst_tiltsecond[3] = cos(gant2_tilt_end)*(-0.160*cos(gant2_rot1)+0.200*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(-0.160*cos(gant2_rot1)+0.110*sin(gant2_rot1));
  gantry2XDimensions_rotfirst_tiltsecond[3] = cos(gant2_tilt_end)*(0.200*cos(gant2_rot1)+0.160*sin(gant2_rot1))+(1-cos(gant2_tilt_end))*(0.110*cos(gant2_rot1)+0.160*sin(gant2_rot1));

  // Final state of gantry1 & gantry2, before rotation with XY limit corners w/ tilt dependence
  gantry1YDimensions_rotsecond_tiltsecond[0] = cos(gant1_tilt_end)*(-0.070*cos(gant1_rot2)-0.200*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(-0.070*cos(gant1_rot2)-0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltsecond[0] = cos(gant1_tilt_end)*(-0.200*cos(gant1_rot2)+0.070*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(-0.110*cos(gant1_rot2)+0.070*sin(gant1_rot2));
  gantry1YDimensions_rotsecond_tiltsecond[1] = cos(gant1_tilt_end)*(-0.070*cos(gant1_rot2)+0.140*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(-0.070*cos(gant1_rot2)+0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltsecond[1] = cos(gant1_tilt_end)*(0.140*cos(gant1_rot2)+0.070*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(0.110*cos(gant1_rot2)+0.070*sin(gant1_rot2));
  gantry1YDimensions_rotsecond_tiltsecond[2] = cos(gant1_tilt_end)*(0.160*cos(gant1_rot2)+0.140*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(0.160*cos(gant1_rot2)+0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltsecond[2] = cos(gant1_tilt_end)*(0.140*cos(gant1_rot2)-0.160*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(0.110*cos(gant1_rot2)-0.160*sin(gant1_rot2));
  gantry1YDimensions_rotsecond_tiltsecond[3] = cos(gant1_tilt_end)*(0.160*cos(gant1_rot2)-0.200*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(0.160*cos(gant1_rot2)-0.110*sin(gant1_rot2));
  gantry1XDimensions_rotsecond_tiltsecond[3] = cos(gant1_tilt_end)*(-0.200*cos(gant1_rot2)-0.160*sin(gant1_rot2))+(1-cos(gant1_tilt_end))*(-0.110*cos(gant1_rot2)-0.160*sin(gant1_rot2));

  gantry2YDimensions_rotsecond_tiltsecond[0] = cos(gant2_tilt_end)*(0.070*cos(gant2_rot2)+0.200*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(0.070*cos(gant2_rot2)+0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltsecond[0] = cos(gant2_tilt_end)*(0.200*cos(gant2_rot2)-0.070*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(0.110*cos(gant2_rot2)-0.070*sin(gant2_rot2));
  gantry2YDimensions_rotsecond_tiltsecond[1] = cos(gant2_tilt_end)*(0.070*cos(gant2_rot2)-0.140*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(0.070*cos(gant2_rot2)-0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltsecond[1] = cos(gant2_tilt_end)*(-0.140*cos(gant2_rot2)-0.070*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(-0.110*cos(gant2_rot2)-0.070*sin(gant2_rot2));
  gantry2YDimensions_rotsecond_tiltsecond[2] = cos(gant2_tilt_end)*(-0.160*cos(gant2_rot2)-0.140*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(-0.160*cos(gant2_rot2)-0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltsecond[2] = cos(gant2_tilt_end)*(-0.140*cos(gant2_rot2)+0.160*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(-0.110*cos(gant2_rot2)+0.160*sin(gant2_rot2));
  gantry2YDimensions_rotsecond_tiltsecond[3] = cos(gant2_tilt_end)*(-0.160*cos(gant2_rot2)+0.200*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(-0.160*cos(gant2_rot2)+0.110*sin(gant2_rot2));
  gantry2XDimensions_rotsecond_tiltsecond[3] = cos(gant2_tilt_end)*(0.200*cos(gant2_rot2)+0.160*sin(gant2_rot2))+(1-cos(gant2_tilt_end))*(0.110*cos(gant2_rot2)+0.160*sin(gant2_rot2));
  **/
  ...
  // Gantry
  std::vector<std::pair<double, double> > gantry1_startpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > gantry1_startpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > gantry1_endpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > gantry1_endpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > gantry2_startpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > gantry2_startpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > gantry2_endpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > gantry2_endpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > gantry1_startpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > gantry1_startpos_rotsecond_tiltsecond;
  std::vector<std::pair<double, double> > gantry1_endpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > gantry1_endpos_rotsecond_tiltsecond;
  std::vector<std::pair<double, double> > gantry2_startpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > gantry2_startpos_rotsecond_tiltsecond;
  std::vector<std::pair<double, double> > gantry2_endpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > gantry2_endpos_rotsecond_tiltsecond;

/**Rika: To be debugged
  // Optical box
  std::vector<std::pair<double, double> > box0_startpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > box0_startpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > box0_endpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > box0_endpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > box1_startpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > box1_startpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > box1_endpos_rotfirst_tiltfirst;
  std::vector<std::pair<double, double> > box1_endpos_rotsecond_tiltfirst;
  std::vector<std::pair<double, double> > box0_startpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > box0_startpos_rotsecond_tiltsecond;
  std::vector<std::pair<double, double> > box0_endpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > box0_endpos_rotsecond_tiltsecond;
  std::vector<std::pair<double, double> > box1_startpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > box1_startpos_rotsecond_tiltsecond;
  std::vector<std::pair<double, double> > box1_endpos_rotfirst_tiltsecond;
  std::vector<std::pair<double, double> > box1_endpos_rotsecond_tiltsecond;
  
  // Relative z positions for collision avoidance calculations (corresponding to indecies in PMT polygon vector)
  double z0_startpos_rotfirst_tiltfirst = (z0_rotfirst_tiltfirst + gantry1ZPos - pmtHeight)/pmtPolyLayerHeight;
  double z0_startpos_rotsecond_tiltfirst = (z0_rotsecond_tiltfirst + gantry1ZPos - pmtHeight)/pmtPolyLayerHeight;
  double z0_endpos_rotfirst_tiltfirst = (z0_rotfirst_tiltfirst + gantry1ZDes - pmtHeight)/pmtPolyLayerHeight;
  double z0_endpos_rotsecond_tiltfirst = (z0_rotsecond_tiltfirst + gantry1ZDes - pmtHeight)/pmtPolyLayerHeight;
  double z1_startpos_rotfirst_tiltfirst = (z1_rotfirst_tiltfirst + gantry2ZPos - pmtHeight)/pmtPolyLayerHeight;
  double z1_startpos_rotsecond_tiltfirst = (z1_rotsecond_tiltfirst + gantry2ZPos - pmtHeight)/pmtPolyLayerHeight;
  double z1_endpos_rotfirst_tiltfirst = (z1_rotfirst_tiltfirst + gantry2ZDes - pmtHeight)/pmtPolyLayerHeight;
  double z1_endpos_rotsecond_tiltfirst = (z1_rotsecond_tiltfirst + gantry2ZDes - pmtHeight)/pmtPolyLayerHeight;
  double z0_startpos_rotfirst_tiltsecond = (z0_rotfirst_tiltsecond + gantry1ZPos- pmtHeight)/pmtPolyLayerHeight;
  double z0_startpos_rotsecond_tiltsecond= (z0_rotsecond_tiltsecond + gantry1ZPos - pmtHeight)/pmtPolyLayerHeight;
  double z0_endpos_rotfirst_tiltsecond = (z0_rotfirst_tiltsecond + gantry1ZDes - pmtHeight)/pmtPolyLayerHeight;
  double z0_endpos_rotsecond_tiltsecond = (z0_rotsecond_tiltsecond + gantry1ZDes - pmtHeight)/pmtPolyLayerHeight;
  double z1_startpos_rotfirst_tiltsecond = (z1_rotfirst_tiltsecond + gantry2ZPos - pmtHeight)/pmtPolyLayerHeight;
  double z1_startpos_rotsecond_tiltsecond= (z1_rotsecond_tiltsecond + gantry2ZPos - pmtHeight)/pmtPolyLayerHeight;
  double z1_endpos_rotfirst_tiltsecond = (z1_rotfirst_tiltsecond + gantry2ZDes - pmtHeight)/pmtPolyLayerHeight;
  double z1_endpos_rotsecond_tiltsecond = (z1_rotsecond_tiltsecond + gantry2ZDes - pmtHeight)/pmtPolyLayerHeight;
  **/

  for (int i = 0; i < 4; ++i){
      // Gantries
    gantry1_startpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry1XPos+gantry1XDimensions_rotfirst_tiltfirst[i], gantry1YPos + gantry1YDimensions_rotfirst_tiltfirst[i]));
    gantry1_startpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry1XPos+gantry1XDimensions_rotsecond_tiltfirst[i], gantry1YPos + gantry1YDimensions_rotsecond_tiltfirst[i]));
    gantry1_endpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry1XDes+gantry1XDimensions_rotfirst_tiltfirst[i], gantry1YDes + gantry1YDimensions_rotfirst_tiltfirst[i]));
    gantry1_endpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry1XDes+gantry1XDimensions_rotsecond_tiltfirst[i], gantry1YDes + gantry1YDimensions_rotsecond_tiltfirst[i]));
    gantry2_startpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry2XPos+gantry2XDimensions_rotfirst_tiltfirst[i], gantry2YPos + gantry2YDimensions_rotfirst_tiltfirst[i]));
    gantry2_startpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry2XPos+gantry2XDimensions_rotsecond_tiltfirst[i], gantry2YPos + gantry2YDimensions_rotsecond_tiltfirst[i]));
    gantry2_endpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry2XDes+gantry2XDimensions_rotfirst_tiltfirst[i], gantry2YDes + gantry2YDimensions_rotfirst_tiltfirst[i]));
    gantry2_endpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry2XDes+gantry2XDimensions_rotsecond_tiltfirst[i], gantry2YDes + gantry2YDimensions_rotsecond_tiltfirst[i]));
    gantry1_startpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry1XPos+gantry1XDimensions_rotfirst_tiltsecond[i], gantry1YPos + gantry1YDimensions_rotfirst_tiltsecond[i]));
    gantry1_startpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry1XPos+gantry1XDimensions_rotsecond_tiltsecond[i], gantry1YPos + gantry1YDimensions_rotsecond_tiltsecond[i]));
    gantry1_endpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry1XDes+gantry1XDimensions_rotfirst_tiltsecond[i], gantry1YDes + gantry1YDimensions_rotfirst_tiltsecond[i]));
    gantry1_endpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry1XDes+gantry1XDimensions_rotsecond_tiltsecond[i], gantry1YDes + gantry1YDimensions_rotsecond_tiltsecond[i]));
    gantry2_startpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry2XPos+gantry2XDimensions_rotfirst_tiltsecond[i], gantry2YPos + gantry2YDimensions_rotfirst_tiltsecond[i]));
    gantry2_startpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry2XPos+gantry2XDimensions_rotsecond_tiltsecond[i], gantry2YPos + gantry2YDimensions_rotsecond_tiltsecond[i]));
    gantry2_endpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry2XDes+gantry2XDimensions_rotfirst_tiltsecond[i], gantry2YDes + gantry2YDimensions_rotfirst_tiltsecond[i]));
    gantry2_endpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry2XDes+gantry2XDimensions_rotsecond_tiltsecond[i], gantry2YDes + gantry2YDimensions_rotsecond_tiltsecond[i]));

    /** Rika: To be debugged
    // Optical box
    box0_startpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry1XPos+opticalBox0XDimensions_rotfirst_tiltfirst[i], gantry1YPos + opticalBox0YDimensions_rotfirst_tiltfirst[i]));
    box0_startpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry1XPos+opticalBox0XDimensions_rotsecond_tiltfirst[i], gantry1YPos + opticalBox0YDimensions_rotsecond_tiltfirst[i]));
    box0_endpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry1XDes+opticalBox0XDimensions_rotfirst_tiltfirst[i], gantry1YDes + opticalBox0YDimensions_rotfirst_tiltfirst[i]));
    box0_endpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry1XDes+opticalBox0XDimensions_rotsecond_tiltfirst[i], gantry1YDes + opticalBox0YDimensions_rotsecond_tiltfirst[i]));

    box1_startpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry2XPos+opticalBox1XDimensions_rotfirst_tiltfirst[i], gantry2YPos + opticalBox1YDimensions_rotfirst_tiltfirst[i]));
    box1_startpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry2XPos+opticalBox1XDimensions_rotsecond_tiltfirst[i], gantry2YPos + opticalBox1YDimensions_rotsecond_tiltfirst[i]));
    box1_endpos_rotfirst_tiltfirst.push_back(std::make_pair(gantry2XDes+opticalBox1XDimensions_rotfirst_tiltfirst[i], gantry2YDes + opticalBox1YDimensions_rotfirst_tiltfirst[i]));
    box1_endpos_rotsecond_tiltfirst.push_back(std::make_pair(gantry2XDes+opticalBox1XDimensions_rotsecond_tiltfirst[i], gantry2YDes + opticalBox1YDimensions_rotsecond_tiltfirst[i]));

    box0_startpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry1XPos+opticalBox0XDimensions_rotfirst_tiltsecond[i], gantry1YPos + opticalBox0YDimensions_rotfirst_tiltsecond[i]));
    box0_startpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry1XPos+opticalBox0XDimensions_rotsecond_tiltsecond[i], gantry1YPos + opticalBox0YDimensions_rotsecond_tiltsecond[i]));
    box0_endpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry1XDes+opticalBox0XDimensions_rotfirst_tiltsecond[i], gantry1YDes + opticalBox0YDimensions_rotfirst_tiltsecond[i]));
    box0_endpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry1XDes+opticalBox0XDimensions_rotsecond_tiltsecond[i], gantry1YDes + opticalBox0YDimensions_rotsecond_tiltsecond[i]));

    box1_startpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry2XPos+opticalBox1XDimensions_rotfirst_tiltsecond[i], gantry2YPos + opticalBox1YDimensions_rotfirst_tiltsecond[i]));
    box1_startpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry2XPos+opticalBox1XDimensions_rotsecond_tiltsecond[i], gantry2YPos + opticalBox1YDimensions_rotsecond_tiltsecond[i]));
    box1_endpos_rotfirst_tiltsecond.push_back(std::make_pair(gantry2XDes+opticalBox1XDimensions_rotfirst_tiltsecond[i], gantry2YDes + opticalBox1YDimensions_rotfirst_tiltsecond[i]));
    box1_endpos_rotsecond_tiltsecond.push_back(std::make_pair(gantry2XDes+opticalBox1XDimensions_rotsecond_tiltsecond[i], gantry2YDes + opticalBox1YDimensions_rotsecond_tiltsecond[i]));
    **/
  }
...
std::cout << "Move z first:  " << move_z1_first << "  " << move_z2_first << std::endl;
  ...
  /**Rika: To be debugged
  // Initialise PMT here
  int polyNum = 1; //TODO: create a forloop later to make 11 polygons and stick them in a vector
  getPMTpolygon(pmtPoly0, pmtRadius, polyNum, pmtXcentre0, pmtYcentre0, numPolySides, pmtPolyLayerHeight);

  pathCalc00.InitialisePMT(pmtPoly0);
  pathCalc01.InitialisePMT(pmtPoly0);
  pathCalc1a.InitialisePMT(pmtPoly0);
  pathCalc1b.InitialisePMT(pmtPoly0);
  pathCalc2a.InitialisePMT(pmtPoly0);
  pathCalc2b.InitialisePMT(pmtPoly0);
  pathCalc3a.InitialisePMT(pmtPoly0);
  pathCalc3b.InitialisePMT(pmtPoly0);
  pathCalc4a.InitialisePMT(pmtPoly0);
  pathCalc4b.InitialisePMT(pmtPoly0);
  pathCalc5a.InitialisePMT(pmtPoly0);
  pathCalc5b.InitialisePMT(pmtPoly0);
  pathCalc6a.InitialisePMT(pmtPoly0);
  pathCalc6b.InitialisePMT(pmtPoly0);
  pathCalc7a.InitialisePMT(pmtPoly0);
  pathCalc7b.InitialisePMT(pmtPoly0);
  pathCalc8a.InitialisePMT(pmtPoly0);
  pathCalc8b.InitialisePMT(pmtPoly0);
  **/
  ...
  // Start, end, and path for gantry 1 move first 
  XYPoint start_rot1 = std::make_pair(gantry1XPos, gantry1YPos);
  XYPoint start_rot2 = std::make_pair(gantry2XPos, gantry2YPos);
  XYPoint end_rot1 = std::make_pair(gantry1XDes, gantry1YDes);
  XYPoint end_rot2 = std::make_pair(gantry2XDes, gantry2YDes);
  ...
  std::pair<double, double> rot11 = std::make_pair(pInfo->Destination[3], pInfo->Destination[3]);
  std::pair<double, double> rot22 = std::make_pair(pInfo->Destination[8], pInfo->Destination[8]);
  ...
  bool goodPath1a = pathCalc1a.CalculatePath(start1, end1, path1, start_tank, end_tank);
  bool goodPath1b = pathCalc1b.CalculatePath(start2, end2, path2, start_tank, end_tank);
  // Calculate good paths for gantry 1 move first, rotate second
  bool goodPath2a = pathCalc2a.CalculatePath(start1, end1, path1, start_tank, end_tank);
  bool goodPath2b = pathCalc2b.CalculatePath(start2, end2, path2, start_tank, end_tank);
  // Calculate good paths for gantry 2 move first, rotate first
  bool goodPath3a = pathCalc3a.CalculatePath(start2, end2, path2, start_tank, end_tank);
  bool goodPath3b = pathCalc3b.CalculatePath(start1, end1, path1, start_tank, end_tank);
  // Calculate good paths for gantry 2 move first, rotate second
  bool goodPath4a = pathCalc4a.CalculatePath(start2, end2, path2, start_tank, end_tank);
  bool goodPath4b = pathCalc4b.CalculatePath(start1, end1, path1, start_tank, end_tank);
  // Incremental rotation path verification 
  bool goodRotation_start_tiltfirst = pathCalc_rotstart_tiltfirst.CalculatePath(start_rot1, start_rot2, rot1, rot2, gant1_tilt_start, gant2_tilt_start, start_tank, end_tank);
  bool goodRotation_end_tiltfirst = pathCalc_rotend_tiltfirst.CalculatePath(end_rot1, end_rot2, rot1, rot2, gant1_tilt_start, gant2_tilt_start, start_tank, end_tank);
  bool goodRotation_endcheck_tiltfirst = pathCalc_rotend_check_tiltfirst.CalculatePath(end_rot1, end_rot2, rot11, rot22, gant1_tilt_start, gant2_tilt_start, start_tank, end_tank);
  /**
   * Made sure to pass by value, since error kept saying:
         feMove.cxx:1568: error: no matching function for call to
         'TRotationCalculator::CalculatePath(XYPoint&, XYPoint&, std::pair<double, double>&, std::pair<double, double>&, double&, double&, std::pair<bool, bool>&, std::pair<bool, bool>&)'
         TRotationCalculator.hxx:29: note: candidates are:
         bool TRotationCalculator::CalculatePath(XYPoint, XYPoint, std::pair<double, double>, std::pair<double, double>, double, double, double, double, std::pair<bool, bool>, std::pair<bool, bool>)
   **/
  // Tilt second
  // Calculate good paths for gantry 1 move first, rotate first
  bool goodPath5a = pathCalc5a.CalculatePath(start1, end1, path1, start_tank, end_tank);
  bool goodPath5b = pathCalc5b.CalculatePath(start2, end2, path2, start_tank, end_tank);
  // Calculate good paths for gantry 1 move first, rotate second
  bool goodPath6a = pathCalc6a.CalculatePath(start1, end1, path1, start_tank, end_tank);
  bool goodPath6b = pathCalc6b.CalculatePath(start2, end2, path2, start_tank, end_tank);
  // Calculate good paths for gantry 2 move first, rotate first
  bool goodPath7a = pathCalc7a.CalculatePath(start2, end2, path2, start_tank, end_tank);
  bool goodPath7b = pathCalc7b.CalculatePath(start1, end1, path1, start_tank, end_tank);
  // Calculate good paths for gantry 2 move first, rotate second
  bool goodPath8a = pathCalc8a.CalculatePath(start2, end2, path2, start_tank, end_tank);
  bool goodPath8b = pathCalc8b.CalculatePath(start1, end1, path1, start_tank, end_tank);
  // Incremental rotation path verification 
  bool goodRotation_start_tiltsecond = pathCalc_rotstart_tiltsecond.CalculatePath(start_rot1, start_rot2, rot1, rot2, gant1_tilt_end, gant2_tilt_end, start_tank, end_tank);
  bool goodRotation_end_tiltsecond = pathCalc_rotend_tiltsecond.CalculatePath(end_rot1, end_rot2, rot1, rot2, gant1_tilt_end, gant2_tilt_end, start_tank, end_tank);
  bool goodRotation_endcheck_tiltsecond = pathCalc_rotend_check_tiltsecond.CalculatePath(end_rot1, end_rot2, rot11, rot22, gant1_tilt_start, gant2_tilt_start, start_tank, end_tank);
  ...
  if(goodPath1a && goodPath1b && goodRotation_start_tiltfirst && goodRotation_endcheck_tiltfirst && !gant2_movefirst){
    ...
    goodPath00 = pathCalc00.CalculatePath(start1, end1, path1, start_tank, end_tank);
    pathCalc01.InitialiseGantries(gantry1_endpos_rotfirst_tiltfirst, gantry2_startpos_rotfirst_tiltfirst);
    goodPath01 = pathCalc01.CalculatePath(start2, end2, path2, start_tank, end_tank);
    move_rotation_first = true;
    tiltfirst = true;
  } else if(goodPath2a && goodPath2b && goodRotation_end_tiltfirst && goodRotation_endcheck_tiltfirst && !gant2_movefirst){
    ...
    goodPath01 = pathCalc01.CalculatePath(start2, end2, path2, start_tank, end_tank);
    tiltfirst = true;
  } else if(goodPath3a && goodPath3b && goodRotation_start_tiltfirst && goodRotation_endcheck_tiltfirst){
    ...
    move_second_gantry_first = true;
    tiltfirst = true;
  } else if(goodPath4a && goodPath4b && goodRotation_end_tiltfirst && goodRotation_endcheck_tiltfirst){
    ...
    move_second_gantry_first = true;
    tiltfirst = true;
  } else if(goodPath5a && goodPath5b && goodRotation_start_tiltsecond && goodRotation_endcheck_tiltsecond && !gant2_movefirst){
    ...
    goodPath01 = pathCalc01.CalculatePath(start2, end2, path2, start_tank, end_tank);
    move_rotation_first = true;
  } else if(goodPath6a && goodPath6b && goodRotation_end_tiltsecond && goodRotation_endcheck_tiltsecond && !gant2_movefirst){
    ...
    goodPath01 = pathCalc01.CalculatePath(start2, end2, path2, start_tank, end_tank);
  } else if(goodPath7a && goodPath7b && goodRotation_start_tiltsecond && goodRotation_endcheck_tiltsecond){
    ...
    move_rotation_first = true;
    move_second_gantry_first = true;
  } else if(goodPath8a && goodPath8b && goodRotation_end_tiltsecond && goodRotation_endcheck_tiltsecond){
...
/*-------getGantryConfigurations-----------------------------------------*/
// Function used in GeneratePath to determine the possible gantry & optical box configurations
// for collision avoidance calculations
//
...
void getGantryConfigurations(double gantryXDimensions[], double gantryYDimensions[], int whichGantry, double rot, double tilt){
    ...
    //Gantry dimensions
    double tiltMotorLength = 0.160; //TODO: REMEASURE
    double gantryFrontHalfLength = 0.140;
    double gantryBackHalfLength = 0.200;
    double gantryOpticalBoxWidth = 0.160;
    double gantryTiltGearWidth = 0.070;
    double gantryOpticalBoxHeight = 0.110;

    if(whichGantry==2){ //Gantry 2 is rotated 180 degrees compared to Gantry 1
	tiltMotorLength *= -1;
	gantryFrontHalfLength *= -1;
	gantryBackHalfLength *= -1;
	gantryOpticalBoxWidth *= -1;
	gantryTiltGearWidth *= -1;
	gantryOpticalBoxHeight *= -1;
    }

    // Calculating gantry configuration, including tilt motor at the top
    gantryXDimensions[0] = cos(tilt)*((-1)*gantryBackHalfLength*cos(rot)+gantryTiltGearWidth*sin(rot))
                           + (1-cos(tilt))*((-1)*gantryOpticalBoxHeight*cos(rot)+gantryTiltGearWidth*sin(rot));
    gantryYDimensions[0] = cos(tilt)*((-1)*gantryTiltGearWidth*cos(rot)-gantryBackHalfLength*sin(rot))
                           + (1-cos(tilt))*((-1)*gantryTiltGearWidth*cos(rot)-gantryOpticalBoxHeight*sin(rot));
    gantryXDimensions[1] = tiltMotorLength*cos(rot) + gantryTiltGearWidth*sin(rot);

    gantryYDimensions[1] = tiltMotorLength*sin(rot) - gantryTiltGearWidth*cos(rot);

    gantryXDimensions[2] = tiltMotorLength*cos(rot) - gantryOpticalBoxWidth*sin(rot);

    gantryYDimensions[2] = tiltMotorLength*sin(rot) + gantryOpticalBoxWidth*cos(rot);

    gantryXDimensions[3] = cos(tilt)*((-1)*gantryBackHalfLength*cos(rot)-gantryOpticalBoxWidth*sin(rot))
                           + (1-cos(tilt))*((-1)*gantryOpticalBoxHeight*cos(rot)-gantryOpticalBoxWidth*sin(rot));
    gantryYDimensions[3] = cos(tilt)*(gantryOpticalBoxWidth*cos(rot)-gantryBackHalfLength*sin(rot))
                           + (1-cos(tilt))*(gantryOpticalBoxWidth*cos(rot)-gantryOpticalBoxHeight*sin(rot));
...
//       collision avoidance control with PMT.

double getGantryConfigurations(double gantryXDimensions[], double gantryYDimensions[], double boxXDimensions[], double boxYDimensions[], int whichGantry, double rot, double tilt){
    ...
    double z_opticalBox = gantryFrontHalfLength; // Lowest z height of optical box; to be calculated
    ...
    if( tilt < 0 ){
	// Z position of optical box is currently independent of tilt;
	// To add tilt dependence, use:
	// z_opticalBox = (-1) * gantryFrontHalfLength * sin(tilt) + gantryOpticalBoxHeight * cos(tilt);
	z_opticalBox = sqrt(gantryFrontHalfLength * gantryFrontHalfLength
			    + gantryOpticalBoxHeight * gantryOpticalBoxHeight);

	// Optical box dimensions currently rotation & tilt independent
	boxXDimensions[0] = sqrt(gantryBackHalfLength * gantryBackHalfLength
                            + gantryOpticalBoxWidth * gantryOpticalBoxWidth 
                            + gantryOpticalBoxHeight * gantryOpticalBoxHeight );
	boxYDimensions[0] = boxXDimensions[0];
	boxXDimensions[1] = boxXDimensions[0];
	boxYDimensions[1] = boxXDimensions[0];
	boxXDimensions[2] = boxXDimensions[0];
	boxYDimensions[2] = boxXDimensions[0];
	boxXDimensions[3] = boxXDimensions[0];
	boxYDimensions[3] = boxXDimensions[0];

    }
    else{
	// To add tilt dependence, use:
	// z_opticalBox = gantryBackHalfLength * sin(tilt) + gantryOpticalBoxHeight * cos(tilt);
	z_opticalBox = sqrt(gantryFrontHalfLength * gantryFrontHalfLength
			    + gantryOpticalBoxHeight * gantryOpticalBoxHeight);
...
void getPMTpolygon(std::vector< std::pair<double, double> > poly, double pmtRadius, int polyNum, double pmtXcentre, double pmtYcentre, int numPolySides, double polyHeight){
    ...
    double radiusOfEnclosingCircle = sqrt(pmtRadius * pmtRadius - zFromCentre * zFromCentre)/ cos(PI / numPolySides);
