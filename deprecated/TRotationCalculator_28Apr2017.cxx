...
//----------------------------------------------------------
TRotationCalculator::TRotationCalculator(double tiltMotorLength, double gantryFrontHalfLength, double gantryBackHalfLength, double gantryOpticalBoxWidth, double gantryTiltGearWidth, double gantryOpticalBoxHeight, double tankCentreXPos, double tankCentreYPos, double tankRadius, double tankPMTholderRadius){
//----------------------------------------------------------
...
//----------------------------------------------------------
void TRotationCalculator::InitialisePMT(std::vector<XYPolygon> pmt0, std::vector<XYPolygon> pmt1, double height, double polygonThickness){
//----------------------------------------------------------
...
//----------------------------------------------------------
bool TRotationCalculator::CalculatePath(XYPoint start0, XYPoint start1, std::pair<double, double> rotationPath0, std::pair<double, double> rotationPath1, std::pair<double, double> tiltPath0, std::pair<double, double> tiltPath1, double gant0_z, double gant1_z, std::pair<bool, bool> tank_height_start, std::pair<bool, bool> tank_height_end){
//----------------------------------------------------------
  ...
  TGantryConfigCalculator gantryConfigCalc(tiltMotor, gFront, gBack, gOpticalBoxWidth, gTiltGearWidth, gOpticalBoxHeight);
  ...
  for(int i = 0; i < maxSteps + 1; i++){
	...
	tilt1 = tilt_end1*pi/180;
      ...
      if(!collisionFree){
          cm_msg(MINFO, "CalculatePath", "Invalid rotation/tilt path: gantries will collide against each other or with tank.");
	  return false;
      }

      if(Z0_lo>=0){ // If Z0_lo>=0, lower surface of optical box 0 is lowered to PMT region, therefore check against PMT for collision.
	  collisionFree = CheckPathForCollisions(&fOpticalBox0_lo, fPMTmultiPoly0.at(Z0_lo), tank_height_start.first, tank_height_end.first);
      }
      if(collisionFree && Z0_up>=0){ // Upper surface of optical box 0 must be checked against PMT for collision.
	  collisionFree = CheckPathForCollisions(&fOpticalBox0_up, fPMTmultiPoly0.at(Z0_up), tank_height_start.first, tank_height_end.first);
      }
      if(collisionFree && Z1_lo>=0){ // If Z1_lo>=0, optical box 1 must be checked against PMT for collision.
	  collisionFree = CheckPathForCollisions(&fOpticalBox1_lo, fPMTmultiPoly1.at(Z1_lo), tank_height_start.second, tank_height_end.second);
      }
      if(collisionFree && Z1_up>=0){ // Upper surface of optical box 0 must be checked against PMT for collision.
	  collisionFree = CheckPathForCollisions(&fOpticalBox1_up, fPMTmultiPoly1.at(Z1_up), tank_height_start.second, tank_height_end.second);
      }
