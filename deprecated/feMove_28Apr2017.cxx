...
#define STORE_DATA(TYPE) {
  ...
  double xtankCentre = 0.360; // Rika (24Apr2017): Updated to estimated new position after tank moved back into coils
  double ytankCentre = 0.345;
  ...
  double pmtXcentre0 = 0.36; // Rika (24Apr2017): Updated to estimated new position.
  double pmtYcentre0 = 0.33;
  double pmtXcentre1 = 0.36;
  double pmtYcentre1 = 0.33;
  int numPolySides = 12;
...
void monitor(HNDLE hDB,HNDLE hKey,void *data){
  ...
  pInfo->Moving = 0; //AJ: This should be a local variable that gets updated:  tempMoving?
  while ((!pInfo->Moving) && i<10){ //AJ: condition on tempMoving?
      pInfo->Moving = pInfo->Moving || pInfo->AxisMoving[i]; //AJ: Set tempMoving?
    i++;
  }

  //pInfo->Moving = tempMoving
  ...
  if(OldMov && !pInfo->Moving){ // i.e. If the motors just stopped moving! AJ:  Should condition on tempMoving! db_set_data and channel_rw can access and change value of pInfo->Moving
    ...
    for(i = gantry_motor_start; i < gantry_motor_end ; i++){
	  ...
	  //break; //no break, because also check for other hit limit switches, and reset their path/destination
	    ...
	    if(!stoppedDueToLimit){//This check is redundant, as 
