...
bool is_move_valid(
  ...
  for (size_t i = 1; i < moving.size(); i++) {
    ...
    if (norm2(dp0) > 0) {
      ...
      if (   intersect(point_to_prisms(pt.gantry0, false)[0], static_geometry, dp0)//TODO: not sure why the staic disp function doesn't work
          || intersect(point_to_prisms(pt.gantry0, false)[1], static_geometry, dp0)
          || intersect(point_to_prisms(pt.gantry0, false)[2], static_geometry, dp0)
      ...
      if (intersect(point_to_optical_box(pt.gantry0, false), static_geometry)
          || intersect(point_to_prisms(pt.gantry1, true)[0], static_geometry, dp1)
          || intersect(point_to_prisms(pt.gantry1, true)[1], static_geometry, dp1)
          || intersect(point_to_prisms(pt.gantry1, true)[2], static_geometry, dp1)) {
        DEBUG_COUT("Found collision.");
        DEBUG_LEAVE;
        return false;
      }
