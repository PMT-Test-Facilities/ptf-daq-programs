...
void move_init(HNDLE hDB,HNDLE hKey,void *data){
  ...
  for(int i = tilt_start; i < tilt_end; i++){
      ...
      if(pInfo->Position[axis] < pInfo->Phidget[7] - tilt_tolerance || pInfo->Position[axis] > pInfo->Phidget[7] + tilt_tolerance){
      	  cm_msg(MERROR, "move_init", "Phidget0%i tilt: %f ODB tilt: %f", i, pInfo->Phidget[7], pInfo->Position[axis]);
      	  cm_msg(MERROR, "move_init", "ERROR: can't start move. Phidget0%i tilt and ODB tilt do no agree within tolerance", i);
      	  return;
      }
      
      // Check if phidget reading is within legal range
      if(pInfo->Phidget[7] < tilt_min || pInfo->Phidget[7] > tilt_max){
	  cm_msg(MERROR, "move_init", "ERROR: can't start move. Phidget0%i tilt: %f is in illegal range", i, pInfo->Phidget[7]);
	  return;
      }
  }
...
int initialize_tilt(INFO *pInfo){
  ...
  while(pInfo->Moving){      
    ...
    // check if motor starts moving but phidget values do not change
    // this may indicate network buffer overflow
  }
