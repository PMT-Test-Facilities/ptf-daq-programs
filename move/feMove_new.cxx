...
#define STORE_DATA(TYPE) {
  ...
  int numPolyLayers = 0.38 / pmtPolyLayerHeight; // Rika (27Apr2017): Inserted enough layers to cover the lowest position that the tip of the optical box can go to
  ...
  TGantryConfigCalculator gantryConfigCalc(tiltMotorLength, gantryFrontHalfLength, gantryBackHalfLength, gantryOpticalBoxWidth, gantryTiltGearWidth, gantryOpticalBoxHeight);
  ...
  TRotationCalculator pathCalc_rot_tilt(tiltMotorLength, gantryFrontHalfLength, gantryBackHalfLength, gantryOpticalBoxWidth, gantryTiltGearWidth, gantryOpticalBoxHeight, xtankCentre, ytankCentre, tankRadius, tankPMTholderRadius);
  ...
  void getPMTpolygon(XYPolygon &poly, double pmtRadius, int polyNum, double pmtXcentre, double pmtYcentre, int numPolySides, double polyHeight);
...
void move_init(HNDLE hDB,HNDLE hKey,void *data){
  ...
  // FOR DEBUGGING
  //cm_msg(MINFO, "move_init", "Function move_init is called.");  
  ...
  double tolerance = 1.;
  // Check if the phidget tilt readings match the ODB variables
  // before proceeding
  // TODO: since error check involves reading phidget, also make sure phidget is still attached and responsive
  // TODO?: maybe first check if they match and if they dont, move it. after moving, if they still
  // don't match throw error and return
  int sizePhidget = sizeof(pInfo->Phidget);  
  db_get_value(pInfo->hDB,pInfo->hKeyPhidget[0],"PH00",&pInfo->Phidget,&sizePhidget,TID_DOUBLE,FALSE);
  if(pInfo->Position[4] < pInfo->Phidget[7] - tolerance || pInfo->Position[4] > pInfo->Phidget[7] + tolerance){
    cm_msg(MERROR, "move_init", "Phidget00 tilt: %f ODB tilt: %f", pInfo->Phidget[7], pInfo->Position[4]);
    //cm_msg(MERROR, "move_init", "Error: Can't start move. ODB tilt and phidget00 tilt do not agree");
    //return;
  }
  db_get_value(pInfo->hDB,pInfo->hKeyPhidget[1],"PH01",&pInfo->Phidget,&sizePhidget,TID_DOUBLE,FALSE);
  if(pInfo->Position[9] < pInfo->Phidget[7] - tolerance || pInfo->Position[9] > pInfo->Phidget[7] + tolerance){
    cm_msg(MERROR, "move_init", "Phidget01 tilt: %f ODB tilt: %f", pInfo->Phidget[7], pInfo->Position[9]); 
    //cm_msg(MERROR, "move_init", "Error: Can't start move. ODB tilt and phidget01 tilt do not agree");
    //return;
  } 
...
int initialize_tilt(INFO *pInfo){
  ...
  for(i = tilt_start; i < tilt_end; i++){
    ...
    //cm_msg(MINFO,"tilt_init","Phidget angle: %f ... ODB Angle: %f", pInfo->Phidget[7], pInfo->Position[axis]); //changed: June 6, 2018
...
int generate_path(INFO *pInfo){
  ...
  double gantry1Tilt = pInfo->Phidget[7];
  cm_msg(MINFO,"generate_path","Current Phidget00 Tilt: %f",gantry1Tilt);

  db_get_value(pInfo->hDB,pInfo->hKeyPhidget[1],"PH01",&pInfo->Phidget,&size,TID_DOUBLE,FALSE);
  double gantry2Tilt = pInfo->Phidget[7];
  cm_msg(MINFO,"generate_path","Current Phidget01 Tilt: %f",gantry2Tilt);
  ...
  double gant1_tilt_end = gantry1Tilt*rad; // changed June 7, 2018
  double gant2_tilt_end = gantry2Tilt*rad; // changed June 7, 2018
  ...
  gantry1_startpos_rotfirst_tiltfirst = gantryConfigCalc.GetGantryConfig(0, gant1_rot1, gant1_tilt_start, gantry1XPos, gantry1YPos);
  gantry1_startpos_rotsecond_tiltfirst = gantryConfigCalc.GetGantryConfig(0, gant1_rot2, gant1_tilt_start, gantry1XPos, gantry1YPos);
  gantry1_endpos_rotfirst_tiltfirst = gantryConfigCalc.GetGantryConfig(0, gant1_rot1, gant1_tilt_start, gantry1XDes, gantry1YDes);
  gantry1_endpos_rotsecond_tiltfirst = gantryConfigCalc.GetGantryConfig(0, gant1_rot2, gant1_tilt_start, gantry1XDes, gantry1YDes);
  box0_startpos_rotfirst_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot1, gant1_tilt_start, gantry1XPos, gantry1YPos);
  box0_startpos_rotsecond_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot2, gant1_tilt_start, gantry1XPos, gantry1YPos);
  box0_endpos_rotfirst_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot1, gant1_tilt_start, gantry1XDes, gantry1YDes);
  box0_endpos_rotsecond_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot2, gant1_tilt_start, gantry1XDes, gantry1YDes);
 // Final state of gantry0 & gantry1, before rotation with XY limit corners with tilt dependence
  gantry2_startpos_rotfirst_tiltfirst = gantryConfigCalc.GetGantryConfig(1, gant2_rot1, gant2_tilt_start, gantry2XPos, gantry2YPos);
  gantry2_startpos_rotsecond_tiltfirst = gantryConfigCalc.GetGantryConfig(1, gant2_rot2, gant2_tilt_start, gantry2XPos, gantry2YPos);
  gantry2_endpos_rotfirst_tiltfirst = gantryConfigCalc.GetGantryConfig(1, gant2_rot1, gant2_tilt_start, gantry2XDes, gantry2YDes);
  gantry2_endpos_rotsecond_tiltfirst = gantryConfigCalc.GetGantryConfig(1, gant2_rot2, gant2_tilt_start, gantry2XDes, gantry2YDes);
  box1_startpos_rotfirst_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot1, gant2_tilt_start, gantry2XPos, gantry2YPos);
  box1_startpos_rotsecond_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot2, gant2_tilt_start, gantry2XPos, gantry2YPos);
  box1_endpos_rotfirst_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot1, gant2_tilt_start, gantry2XDes, gantry2YDes);
  box1_endpos_rotsecond_tiltfirst = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot2, gant2_tilt_start, gantry2XDes, gantry2YDes);
  //Tilt second
  //Final state of gantry0 & gantry1, after rotation with XY limit corners with tilt dependence
  gantry1_startpos_rotfirst_tiltsecond = gantryConfigCalc.GetGantryConfig(0, gant1_rot1, gant1_tilt_end, gantry1XPos, gantry1YPos);
  gantry1_startpos_rotsecond_tiltsecond = gantryConfigCalc.GetGantryConfig(0, gant1_rot2, gant1_tilt_end, gantry1XPos, gantry1YPos);
  gantry1_endpos_rotfirst_tiltsecond = gantryConfigCalc.GetGantryConfig(0, gant1_rot1, gant1_tilt_end, gantry1XDes, gantry1YDes);
  gantry1_endpos_rotsecond_tiltsecond = gantryConfigCalc.GetGantryConfig(0, gant1_rot2, gant1_tilt_end, gantry1XDes, gantry1YDes);
  box0_startpos_rotfirst_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot1, gant1_tilt_end, gantry1XPos, gantry1YPos);
  box0_startpos_rotsecond_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot2, gant1_tilt_end, gantry1XPos, gantry1YPos);
  box0_endpos_rotfirst_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot1, gant1_tilt_end, gantry1XDes, gantry1YDes);
  box0_endpos_rotsecond_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(0, gant1_rot2, gant1_tilt_end, gantry1XDes, gantry1YDes);
  // Final state of gantry0 & gantry1, before rotation with XY limit corners with tilt dependence
  gantry2_startpos_rotfirst_tiltsecond = gantryConfigCalc.GetGantryConfig(1, gant2_rot1, gant2_tilt_end, gantry2XPos, gantry2YPos);
  gantry2_startpos_rotsecond_tiltsecond = gantryConfigCalc.GetGantryConfig(1, gant2_rot2, gant2_tilt_end, gantry2XPos, gantry2YPos);
  gantry2_endpos_rotfirst_tiltsecond = gantryConfigCalc.GetGantryConfig(1, gant2_rot1, gant2_tilt_end, gantry2XDes, gantry2YDes); 
  gantry2_endpos_rotsecond_tiltsecond = gantryConfigCalc.GetGantryConfig(1, gant2_rot2, gant2_tilt_end, gantry2XDes, gantry2YDes);
  box1_startpos_rotfirst_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot1, gant2_tilt_end, gantry2XPos, gantry2YPos);
  box1_startpos_rotsecond_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot2, gant2_tilt_end, gantry2XPos, gantry2YPos);
  box1_endpos_rotfirst_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot1, gant2_tilt_end, gantry2XDes, gantry2YDes); 
  box1_endpos_rotsecond_tiltsecond = gantryConfigCalc.GetOpticalBoxConfig(1, gant2_rot2, gant2_tilt_end, gantry2XDes, gantry2YDes);
  ...
  if(pInfo->Destination[0] < pInfo->LimPos[0] || pInfo->Destination[0] > pInfo ->LimPos[5]){
    cm_msg(MERROR, "generate_path", "Gantry 0 X destination at %5.3f is outside gantry limits: not between %5.3f and %5.3f",
  ...
  if(pInfo->Destination[5] < pInfo->LimPos[0] || pInfo->Destination[5] > pInfo ->LimPos[5]){
    cm_msg(MERROR, "generate_path", "Gantry 1 X destination at %5.3f is outside gantry limits: not between %5.3f and %5.3f",
  ...
  if(pInfo->Destination[1] < pInfo->LimPos[1] || pInfo->Destination[1] > pInfo ->LimPos[6]){
    cm_msg(MERROR, "generate_path", "Gantry 0 Y destination at %5.3f is outside gantry limits: not between %5.3f and %5.3f",
  ...
  if(pInfo->Destination[6] < pInfo->LimPos[1] || pInfo->Destination[6] > pInfo ->LimPos[6]){
    cm_msg(MERROR, "generate_path", "Gantry 1 Y destination at %5.3f is outside gantry limits: not between %5.3f and %5.3f",
...
std::cout << "Move z first:  " << move_z1_first << "  " << move_z2_first << std::endl;
  ...
  // Check gantries & tank:
  validDestination_box0 = pathCalc_checkDestination.CheckDestination(gantry1_endpos_rotfirst_tiltfirst, gantry2_endpos_rotfirst_tiltfirst, tankheight_gantend1);
  validDestination_box1 = pathCalc_checkDestination.CheckDestination(gantry2_endpos_rotfirst_tiltfirst, gantry1_endpos_rotfirst_tiltfirst, tankheight_gantend2);
  ...
  // Check PMT:
  if(finalZ_box0_lo >= 0){
      validDestination_box0 = pathCalc_checkDestination.CheckDestination(box0_endpos_rotfirst_tiltfirst.first, pmtPoly0.at(finalZ_box0_lo), tankheight_gantend1);
      if(validDestination_box0 && finalZ_box0_up>=0){
          validDestination_box0 = pathCalc_checkDestination.CheckDestination(box0_endpos_rotfirst_tiltfirst.second, pmtPoly0.at(finalZ_box0_up), tankheight_gantend1);
      }
  }
  if(finalZ_box1_lo >= 0){
      validDestination_box1 = pathCalc_checkDestination.CheckDestination(box1_endpos_rotfirst_tiltfirst.first, pmtPoly1.at(finalZ_box1_lo), tankheight_gantend2);
      if(validDestination_box1 && finalZ_box1_up>=0){
          validDestination_box1 = pathCalc_checkDestination.CheckDestination(box1_endpos_rotfirst_tiltfirst.second, pmtPoly1.at(finalZ_box1_up), tankheight_gantend2);
      }
  }
  if(!(validDestination_box0 && validDestination_box1)){
  ...
  std::pair<double, double> tilt_path1 = std::make_pair(gantry1Tilt, pInfo->Destination[4]);
  std::pair<double, double> tilt_path2 = std::make_pair(gantry2Tilt, pInfo->Destination[9]);
  std::pair<double, double> tilt_start1 = std::make_pair(gantry1Tilt, gantry1Tilt);
  std::pair<double, double> tilt_start2 = std::make_pair(gantry2Tilt, gantry2Tilt);
  std::pair<double, double> tilt_end1 = std::make_pair(pInfo->Destination[4], pInfo->Destination[4]);
  std::pair<double, double> tilt_end2 = std::make_pair(pInfo->Destination[9], pInfo->Destination[9]);
  ...
  bool goodRotationTilt_rotfirst_tiltfirst = pathCalc_rot_tilt.CalculatePath(xy_start1, xy_start2, rot_path1, rot_path2, tilt_path1, tilt_path2, gantry1Z, gantry2Z, start_tank, end_tank);
  bool goodTilt_rotsecond_tiltfirst = pathCalc_rot_tilt.CalculatePath(xy_start1, xy_start2, rot_start1, rot_start2, tilt_path1, tilt_path2, gantry1Z, gantry2Z, start_tank, end_tank);
  bool goodRotation_rotsecond_tiltfirst = pathCalc_rot_tilt.CalculatePath(xy_end1, xy_end2, rot_path1, rot_path2, tilt_end1, tilt_end2, gantry1Z, gantry2Z, start_tank, end_tank);
  bool goodRotationTilt_endcheck_tiltfirst = pathCalc_rot_tilt.CalculatePath(xy_end1, xy_end2, rot_end1, rot_end2, tilt_end1, tilt_end2, gantry1Z, gantry2Z, start_tank, end_tank);
  // Tilt second
  // Calculate good paths for gantry 1 move first, rotate first
  ...
  bool goodRotation_rotfirst_tiltsecond = pathCalc_rot_tilt.CalculatePath(xy_start1, xy_start2, rot_path1, rot_path2, tilt_start1, tilt_start2, gantry1Z, gantry2Z, start_tank, end_tank);
  bool goodTilt_rotfirst_tiltsecond = pathCalc_rot_tilt.CalculatePath(xy_end1, xy_end2, rot_end1, rot_end2, tilt_path1, tilt_path2, gantry1Z, gantry2Z, start_tank, end_tank);
  bool goodRotationTilt_rotsecond_tiltsecond = pathCalc_rot_tilt.CalculatePath(xy_end1, xy_end2, rot_path1, rot_path2, tilt_path1, tilt_path2, gantry1Z, gantry2Z, start_tank, end_tank);
  bool goodRotationTilt_endcheck_tiltsecond = pathCalc_rot_tilt.CalculatePath(xy_end1, xy_end2, rot_end1, rot_end2, tilt_end1, tilt_end2, gantry1Z, gantry2Z, start_tank, end_tank);
  ...
  if(goodPath1a && goodPath1b && goodRotationTilt_rotfirst_tiltfirst && goodRotationTilt_endcheck_tiltfirst && !gant2_movefirst){
  ...
  else if(goodPath2a && goodPath2b && goodTilt_rotsecond_tiltfirst && goodRotation_rotsecond_tiltfirst && goodRotationTilt_endcheck_tiltfirst && !gant2_movefirst){
  ...
  else if(goodPath4a && goodPath4b && goodTilt_rotsecond_tiltfirst && goodRotation_rotsecond_tiltfirst && goodRotationTilt_endcheck_tiltfirst){
  ...
  else if(goodPath5a && goodPath5b && goodRotation_rotfirst_tiltsecond && goodTilt_rotfirst_tiltsecond && goodRotationTilt_endcheck_tiltsecond && !gant2_movefirst){
  ...
  else if(goodPath6a && goodPath6b && goodRotationTilt_rotsecond_tiltsecond && goodRotationTilt_endcheck_tiltsecond && !gant2_movefirst){
      ...
      pathCalc00.InitialiseOpticalBoxes(box0_startpos_rotsecond_tiltsecond.first, box0_startpos_rotsecond_tiltsecond.second,
					box1_startpos_rotsecond_tiltsecond.first, box1_startpos_rotsecond_tiltsecond.second,
					opticalBox0_lo_Z[1], opticalBox0_up_Z[1], opticalBox1_lo_Z[1], opticalBox1_up_Z[1]);
      goodPath00 = pathCalc00.CalculatePath(xy_start1, xy_end1, path1, start_tank, end_tank);
  ...
  else if(goodPath7a && goodPath7b && goodRotation_rotfirst_tiltsecond && goodTilt_rotfirst_tiltsecond && goodRotationTilt_endcheck_tiltsecond){
  ...
  else if(goodPath8a && goodPath8b && goodRotationTilt_rotsecond_tiltsecond && goodRotationTilt_endcheck_tiltsecond){
      ...
      pathCalc01.InitialiseOpticalBoxes(box0_startpos_rotsecond_tiltsecond.first, box0_startpos_rotsecond_tiltsecond.second,
					box1_endpos_rotsecond_tiltsecond.first, box1_endpos_rotsecond_tiltsecond.second,
					opticalBox0_lo_Z[1], opticalBox0_up_Z[1], opticalBox1_lo_Z[1], opticalBox1_up_Z[1]);
      goodPath01 = pathCalc01.CalculatePath(xy_start2, xy_end2, path2, start_tank, end_tank);
      // Check first whether we are not crossing a beam in a way that gantry1 had to move first
...
void move(INFO *pInfo) {
  ...
  for(i = gantry_motor_start; i < gantry_motor_end ; i++){
    ...
    printf("move","MD[%i]=%6.0f(%6.0f) ",i,pInfo->CountDest[i],pInfo->CountPos[i]);
    if(i==4 || i == 9){
      printf("\n");
    }
...
void getPMTpolygon(XYPolygon &poly, double pmtRadius, int polyNum, double pmtXcentre, double pmtYcentre, int numPolySides, double polyHeight){
    ...
    double radiusOfEnclosingCircle = sqrt(pmtRadius * pmtRadius - zFromCentre * zFromCentre)/ cos(PI / numPolySides) + 0.001;
