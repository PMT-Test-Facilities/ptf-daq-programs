...
int main(void) {
  ...
  const auto
    ...
    cerr<< dg1a[0] <<endl;
    const vector<PhidgetINFO> phidgets = {{3,P_USB},{4,P_USB},{0,P_VINT},{1,P_VINT},{2,P_VINT}};//Have to allow measurements from VINT phidget
  ...
  for (int i = 0; i < v0s.size(); i++) {
    ...
    for (int j = 0; j < v1s.size(); j++) {
      ...
      for (int _i = 0; _i < N_SAMPLES; _i++) {
        ...
        auto mp_dn = iface.get_phidget_reading({4,P_USB}); 
        auto mp_up = iface.get_phidget_reading({0,P_VINT});
        auto mp_sd = iface.get_phidget_reading({1,P_VINT});
