...
//	printf("Status %s", response);
  ...
  response[buffLength] = 0x0;
  if ((strchr(response, ':') == NULL) || (strchr(response, '?') != NULL)) {
      cm_msg(MINFO, "galil_read", "Invalid string length or error on RP read. Read %u chars. Response %s Rejected",buffLength, response);
    goto CompletedRP;
  }
...
//		pInfo->iPosition[2], pInfo->iPosition[3]);
  ...
  printf("Encoder Read Function Here");
...
INT galil_init(EQUIPMENT *pequipment) {
  ...
  // check encoder status
   printf("Encoder Test\n");
   sprintf(command, "CEF 0 ");

 /*
 sprintf(command, "CEF ? \r");
